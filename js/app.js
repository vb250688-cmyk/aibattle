    ...
    if(!hardBlockLong){
      ...
      if(canLong){
        ...
        const longOk=canLong&&bearLongAllowed&&longScore>=entryThresh
          ...
          &&ind.rsi>18&&ind.rsi<86&&t6LongOk&&longMomOk;
...
// ══════════════════════════════════════════════════════════════
// PHASE 3 ADDITIONS — Web3: $BATTLE Token, NFT Badges, Staking
// (Everything below is NEW code. Nothing above this line was
//  removed or altered in its calculation/behavior.)
//
// Requires ethers.js (loaded via CDN in index.html) and the three
// contracts in /contracts deployed to Sepolia — see DEPLOY_GUIDE.md.
// Until WEB3_CONFIG addresses are filled in, the panel shows a
// "not deployed yet" message instead of erroring.
// ══════════════════════════════════════════════════════════════

let _web3 = { provider:null, signer:null, token:null, nft:null, staking:null, ready:false };

function _isWeb3Deployed(){
  const w = (typeof WEB3_CONFIG!=='undefined') ? WEB3_CONFIG : null;
  if(!w) return false;
  const zero = '0x0000000000000000000000000000000000000000';
  return w.tokenAddress!==zero && w.nftAddress!==zero && w.stakingAddress!==zero;
}

async function _ensureWeb3Ready(){
  if(typeof ethers==='undefined'){
    showToast('⚠️ ethers.js not loaded — check your internet connection.','error');
    return false;
  }
  if(!walletAddress){
    showToast('🔗 Pehle wallet connect karo (CONNECT WALLET button)','warning');
  ...
  if(!_isWeb3Deployed()){
    showToast('⚠️ Web3 contracts not deployed yet. See DEPLOY_GUIDE.md','warning');
    return false;
  }
  await ensureSepolia();
  const rawProvider = getMetaMaskProvider() || window.ethereum;
  if(!rawProvider){ showToast('MetaMask not found!','error'); return false; }
  try{
    _web3.provider = new ethers.BrowserProvider(rawProvider);
    _web3.signer   = await _web3.provider.getSigner();
    _web3.token    = new ethers.Contract(WEB3_CONFIG.tokenAddress, BATTLE_TOKEN_ABI, _web3.signer);
    _web3.nft      = new ethers.Contract(WEB3_CONFIG.nftAddress, BATTLE_NFT_ABI, _web3.signer);
    _web3.staking  = new ethers.Contract(WEB3_CONFIG.stakingAddress, BATTLE_STAKING_ABI, _web3.signer);
    _web3.ready = true;
    return true;
  }catch(e){
    showToast('⚠️ Web3 init failed: '+(e.message||e),'error');
    return false;
  }
}

// ── OPEN / CLOSE WEB3 HUB MODAL ─────────────────────────────────
async function openWeb3Hub(){
  const modal = document.getElementById('web3-hub-modal');
  if(!modal) return;
  modal.style.display = 'flex';
  await refreshWeb3Hub();
}
function closeWeb3Hub(){
  const modal = document.getElementById('web3-hub-modal');
  if(modal) modal.style.display = 'none';
}

async function refreshWeb3Hub(){
  const body = document.getElementById('web3-hub-body');
  if(!body) return;

  if(!walletAddress){
    body.innerHTML = `<div style="text-align:center;color:var(--text2);padding:24px;font-size:12px;">🔗 Connect your wallet first (CONNECT WALLET button top of screen).</div>`;
    return;
  }
  if(!_isWeb3Deployed()){
    body.innerHTML = `<div style="text-align:center;color:var(--text2);padding:24px;font-size:12px;">
      ⚠️ Web3 contracts not deployed yet.<br><br>
      Deploy <code>contracts/BattleToken.sol</code>, <code>BattleNFT.sol</code> and
      <code>BattleStaking.sol</code> to Sepolia (see <b>DEPLOY_GUIDE.md</b>), then paste
      the 3 addresses into <code>WEB3_CONFIG</code> at the top of <code>js/config.js</code>.
    </div>`;
    return;
  }

  body.innerHTML = `<div style="text-align:center;color:var(--text2);padding:20px;font-size:12px;">⏳ Loading on-chain data...</div>`;
  const ok = await _ensureWeb3Ready();
  if(!ok) return;

  try{
    const [balRaw, decimals, pendingRaw, stakeInfo, aprBps, badgeOwned] = await Promise.all([
      _web3.token.balanceOf(walletAddress),
      _web3.token.decimals(),
      _web3.staking.pendingReward(walletAddress),
      _web3.staking.stakes(walletAddress),
      _web3.staking.aprBasisPoints(),
      Promise.all(BADGE_DEFS.map(b => _web3.nft.owns(walletAddress, b.id).catch(()=>false))),
    ]);

    const bal = Number(ethers.formatUnits(balRaw, decimals));
    const pending = Number(ethers.formatUnits(pendingRaw, decimals));
    const staked = Number(ethers.formatUnits(stakeInfo.amount ?? stakeInfo[0], decimals));
    const apr = Number(aprBps) / 100;

    // ── Badges grid ──
    const badgesHtml = BADGE_DEFS.map((b,i)=>{
      const owned = badgeOwned[i];
      const unlockedInGame = b.check(state);
      let action;
      if(owned){
        action = `<span style="color:var(--green);font-size:9px;">✅ MINTED</span>`;
      } else if(unlockedInGame){
        action = `<button class="btn-sm" style="font-size:9px;padding:3px 6px;" onclick="mintBadgeNFT(${b.id})">✨ MINT</button>`;
      } else {
        action = `<span style="color:var(--text2);font-size:9px;">🔒 LOCKED</span>`;
      }
      return `<div style="display:flex;align-items:center;justify-content:space-between;padding:6px 8px;border:1px solid var(--border);border-radius:8px;margin-bottom:6px;${owned?'opacity:1':'opacity:0.85'}">
        <span style="font-size:11px;">${b.icon} ${b.name}</span>${action}
      </div>`;
    }).join('');

    body.innerHTML = `
      <div style="border:1px solid var(--border);border-radius:8px;padding:10px;margin-bottom:12px;">
        <div style="font-size:10px;color:var(--text2);margin-bottom:4px;">🪙 $BATTLE BALANCE</div>
        <div style="font-size:20px;color:var(--cyan);font-weight:700;">${bal.toFixed(2)}</div>
        <button class="wl-modal-btn primary" style="margin-top:8px;margin-bottom:0;" onclick="claimDailyBattle()">🎁 CLAIM DAILY (100 BATTLE)</button>
      </div>

      <div style="border:1px solid var(--border);border-radius:8px;padding:10px;margin-bottom:12px;">
        <div style="font-size:10px;color:var(--text2);margin-bottom:4px;">📈 STAKING (APR ${apr}%)</div>
        <div style="font-size:12px;margin-bottom:6px;">Staked: <b style="color:var(--cyan)">${staked.toFixed(2)}</b> BATTLE &nbsp;|&nbsp; Pending: <b style="color:var(--green)">${pending.toFixed(4)}</b> BATTLE</div>
        <div style="display:flex;gap:4px;margin-bottom:6px;">
          <input type="number" id="web3-stake-amt" placeholder="Amount" min="0" style="flex:1;">
          <button class="btn-sm" style="font-size:9px;" onclick="stakeBattle()">STAKE</button>
          <button class="btn-sm" style="font-size:9px;" onclick="unstakeBattle()">UNSTAKE</button>
        </div>
        <button class="wl-modal-btn ghost" style="margin-bottom:0;" onclick="claimStakingReward()">💰 CLAIM STAKING REWARD</button>
      </div>

      <div style="font-size:10px;color:var(--text2);margin-bottom:6px;">🏅 ACHIEVEMENT BADGES (NFT)</div>
      ${badgesHtml}
    `;
  }catch(e){
    body.innerHTML = `<div style="text-align:center;color:var(--red);padding:20px;font-size:11px;">⚠️ Failed to load: ${e.message||e}</div>`;
  }
}

// ── ACTIONS ──────────────────────────────────────────────────────
async function claimDailyBattle(){
  if(!(await _ensureWeb3Ready())) return;
  try{
    showToast('⏳ Sending claim tx...','info');
    const tx = await _web3.token.claimDaily();
    await tx.wait();
    showToast('🎉 100 BATTLE claimed!','success');
    refreshWeb3Hub();
  }catch(e){
...
async function mintBadgeNFT(badgeType){
  if(!(await _ensureWeb3Ready())) return;
  const def = BADGE_DEFS.find(b=>b.id===badgeType);
  if(!def || !def.check(state)){
    showToast('🔒 Badge abhi unlock nahi hui in-game.','warning');
    return;
  }
  try{
    showToast(`⏳ Minting ${def.name} badge...`,'info');
    const tx = await _web3.nft.mintBadge(badgeType);
    await tx.wait();
    showToast(`🏅 ${def.name} badge minted!`,'success');
    refreshWeb3Hub();
  }catch(e){
    showToast('⚠️ Mint failed: '+(e.reason||e.message||e),'error');
  }
}

async function stakeBattle(){
  if(!(await _ensureWeb3Ready())) return;
  ...
  try{
    ...
    const approveTx = await _web3.token.approve(WEB3_CONFIG.stakingAddress, amtWei);
    await approveTx.wait();
    showToast('⏳ Staking...','info');
    const stakeTx = await _web3.staking.stake(amtWei);
    await stakeTx.wait();
    showToast(`✅ Staked ${amt} BATTLE`,'success');
    if(input) input.value='';
    refreshWeb3Hub();
  }catch(e){
    showToast('⚠️ Stake failed: '+(e.reason||e.message||e),'error');
  }
}
