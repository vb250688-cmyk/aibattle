...
function checkAITrade(){
  ...
  for(const coin of COINS){
    ...
    const MIN_TP=0.45;  // 0.45% — covers fees with room for real profit
    ...
    if(!hardBlockShort && aiMemory.shortsWorking()){
      ...
      if(canShort){
        ...
        if(shortOk){
          ...
          const margin=Math.max(120,Math.min(Math.floor(riskAmt*1.4),state.aiCash*0.55,280));
      ...
      if(canLong){
        ...
        if(longOk){
          ...
          if(longScore < longThresh){ /* coin preference says skip */ }
            ...
            const amount=Math.max(120,Math.min(Math.floor(riskAmt*1.4),state.aiCash*0.65,280));
