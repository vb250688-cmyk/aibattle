...
function checkAITrade(){
  ...
  for(const coin of COINS){
    ...
    const MIN_TP=0.40;  // lowered: min TP = 0.4% (fees are smaller now)
    const MIN_SL=0.50;  // min SL = 0.5%
    ...
    if(short){
      ...
      const exitShort=
        ...
        || (holdSec>360&&pct<0)                       // 6min if losing
        || (holdSec>600&&pct<FEE_RT);              // 10min if barely profitable
...
function executeAITrade(side, amount, coinId, meta={}) {
  ...
  if(side==='buy'){
    ...
    const exitFee = margin * 0.001;  // ✅ FIX: fee on margin (was notional → 20x too high)
