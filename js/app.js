...
function initFuturesWs(){
  ...
  // ★ FIX: previously called initWs() here, which also tore down/recreated
  // the SPOT connection every time just the FUTURES stream dropped. Over
  // repeated futures reconnects this piled up duplicate spot WebSocket
  // connections in the background. Now each stream reconnects itself only.
  wsFutures.onclose=()=>setTimeout(initFuturesWs,3000);
}

function initWs(){ initSpotWs(); initFuturesWs(); }
initWs();
...
function drawChart(){
  ...
  // ★ FIX: canvas was rendering blurry on Retina/high-DPI screens because
  // width/height were set to CSS pixel values with no devicePixelRatio
  // scaling. Backing-store now matches actual device pixels, then we scale
  // the drawing context back down so all existing coordinate math (which
  // assumes W×H) still works unchanged.
...
function checkAITrade(){
  ...
  const _forcedMode = _msSinceLastTrade > 240000; // 4 minutes with no trade
  const _forcedThreshReduction = _forcedMode ? 3 : 0; // reduce all thresholds by 3 in forced mode
  ...
  for(const coin of COINS){
    ...
    // ★ FIX (v2 — reverts the v1 fix above this comment):
    // v1 removed the wallet check here because at the time the Supabase
    // Edge Function (ai-trade-tick) was never actually being invoked (no
    // cron set up), so gating local entries on walletAddress meant NO
    // entries happened at all once a wallet connected.
    // That cron is now confirmed running every minute (verified via
    // Invocations tab). With BOTH local entries and the cron edge function
    // deciding trades independently for the same wallet, every open
    // browser tab / device was fighting the cron (and each other) to
    // overwrite the same Supabase row — causing points/trades to diverge
    // between devices, and a "trade" firing the instant the site opened
    // (that was just this device's own local entry, not the cron's).
    // Restoring the wallet gate makes the cron the single source of truth
    // for entries once a wallet is connected; local play (no wallet) is
    // unaffected. Exits stay local/unconditional below — closing a
    // position that's already been closed elsewhere is a safe no-op.
    ...
    if(short){
      ...
      const exitShort=
        ...
        || (holdSec>360&&pct<-(FEE_RT*0.5))          // 6min if noticeably losing
        || (holdSec>720&&pct<=0);                  // 12min only if still at loss/breakeven
    ...
    if(!hardBlockShort && aiMemory.shortsWorking()){
      ...
      if(canShort){
        ...
        const shortOk = shortScore>=shortThresh
          ...
          &&ind.htfTrend!==1
          &&ind.rsi>20
          &&shortAdxOk&&t6ShortOk&&shortMomOk;
    ...
    if(!hardBlockLong){
      ...
      if(canLong){
        ...
        // ★ FIX: RSI floor was 18 — blocked exactly the deep-oversold bounce
        // entries this strategy is meant to catch (e.g. RSI:16 showed "BUY"
        // on screen but was silently rejected here). The real falling-knife
        // protection is longMomOk (actual price momentum via roc10), so the
        // static RSI floor only needs to guard against literal broken/zero
        // readings now, not block legitimate deep-oversold setups.
        const longOk=canLong&&bearLongAllowed&&longScore>=entryThresh
          ...
          &&ind.rsi>5&&ind.rsi<86&&t6LongOk&&longMomOk;
...
function executeAITrade(side, amount, coinId, meta={}) {
  ...
  if(!price)return;
  const time=new Date().toLocaleTimeString('en-IN',{timeZone:'Asia/Kolkata',hour12:true,hour:'2-digit',minute:'2-digit',second:'2-digit'});
...
function importSave(file){
  ...
  // ★ FIX: HTML's <input onchange="importSave(this.files[0])"> already passes
  // the selected file — previously this param was ignored and a brand-new
  // file picker was opened instead, forcing the user to pick the file twice.
...
function updateUI(){
  // Win/loss counter badges (also used as celebration-flash targets)
  {const e=document.getElementById('sc-ai-pts');if(e)e.textContent=`W${state.aiWins||0} / L${state.aiLosses||0}`;}
  {const e=document.getElementById('sc-user-pts');if(e)e.textContent=`W${state.userWins||0} / L${state.userLosses||0}`;}
  ...
  logItems=logItems.concat(state.aiTradeLog.slice(0,5).map(t=>{
    ...
    return `<div class="ai-trade-item ${escHtml(t.type)} ${cls}">${icon} ${escHtml(t.time)} ${levStr}${escHtml(String(t.type).toUpperCase())} ${t.qty.toFixed(4)} ${escHtml(t.coin)} @$${fmtPrice(t.price)}${pnlStr}</div>`;
  }));
...
// ★ Defense-in-depth: trade-log fields (coin id, type, time) normally come
// from our own fixed COINS list / internal enums, so they're safe today.
// But some of this data round-trips through Supabase (cloud sync) using a
// public/publishable key, so if RLS is ever misconfigured, a malicious
// writer could inject arbitrary strings into a synced row. Escaping before
// interpolating into innerHTML costs nothing and closes that door.
function escHtml(s){
...
// ★ FIX: lifetimePts only ever goes UP (points are added on wins only, never
// subtracted on losses — see addAiPoints/addUserPoints), so the old
// "lifetimePts.ai < _prevAiPts" loss-detection condition below could never
// be true. Track losses via the actual loss counters instead.
let _prevAiLosses=state.aiLosses||0, _prevUserLosses=state.userLosses||0;
...
function checkWinEvents(){
  ...
  // Loss detection — now correctly tracks state.aiLosses/userLosses instead
  // of a lifetimePts comparison that could never fire (points never decrease).
  if(state.aiLosses>_prevAiLosses){
    const lastT=state.aiTradeLog.find(t=>t.pnl!=null&&t.pnl<0);
    if(lastT)celebrateLoss(false,lastT.pnl);
    _prevAiLosses=state.aiLosses;
  }
  if(state.userLosses>_prevUserLosses){
    const lastT=state.userTradeLog.find(t=>t.pnl!=null&&t.pnl<0);
    if(lastT)celebrateLoss(true,lastT.pnl);
    _prevUserLosses=state.userLosses;
  }
...
function notify(msg,type,evt){
  // ★ FIX: previously overwrote a single shared #notif element's textContent
  // every call — with AI trading every 1-2s, rapid-fire notifications just
  // replaced each other and most were never actually seen. CSS already had
  // .notif-item stacking styles defined (flex-column container + individual
  // animated items) but JS never used them. Now each call appends its own
  // toast that removes itself after its timeout, so they stack and are all
  // visible.
...
(async function init(){
  ...
  // Poll Supabase for background AI trades (Edge Function ai-trade-tick runs
  // independently on a cron schedule — this pulls its results into the UI
  // every 20s while a wallet is connected, so trades made while the app was
  // closed/backgrounded show up here without needing a manual refresh)
  setInterval(async()=>{
    if(!walletAddress) return;
    try{
      const row = await supabaseLoad();
      if(row) applySupaRow(row);
    }catch(e){ console.warn('[BG-AI poll] failed:', e.message); }
  }, 20000);
...
async function ensureSepolia(providedEth){
  // ★ FIX: was called as ensureSepolia(eth) from connectMetaMask but took no
  // parameters, silently ignoring the already-obtained provider and calling
  // getMetaMaskProvider() again. Harmless when there's only one provider,
  // but risks acting on the wrong wallet if multiple extensions are
  // installed. Now uses the passed provider when given.
  const eth = providedEth || getMetaMaskProvider();
  if(!eth) return;
  try{
...
function buildSupaSnap(){
  ...
  const positions = {
    ...
    unlockedTiers : state.unlockedTiers || []  // needed by background Edge Function AI
  };
  return {
...
function applySupaRow(row){
  ...
  try{
    ...
    // Merge unlockedTiers from cloud (union — never lose a tier unlocked on another device)
    if(p.unlockedTiers){
      state.unlockedTiers = Array.from(new Set([...(state.unlockedTiers||[]), ...p.unlockedTiers]));
      saveTiers();
    }
...
async function unlockTier(tierId){
  ...
  try {
    // ★ FIX: previously sent to CONTRACT_ADDRESS, which reverts on-chain for
    // EVERY tier (it's a contract address with no fallback function that
    // accepts arbitrary calldata, so any tx with a `data` payload bounces).
    // We only need a real Sepolia gas cost — not an actual contract call —
    // so we self-send to the connected wallet instead. Self-sends to an EOA
    // never revert, while the tierId is still recorded on-chain in `data`.
    const txHash = await provider.request({
      ...
      params: [{
        ...
        to     : walletAddress,
        value  : '0x0',
        data   : '0x' + tierId.toString(16).padStart(8,'0'),
