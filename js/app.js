...
function checkAITrade(){
  ...
  for(const coin of COINS){
    ...
    const atrTP=Math.max(MIN_TP, ind.atrPct*(ind.strongTrend?1.6:1.1)*_levDeflator);
    const atrSL=Math.max(MIN_SL, ind.atrPct*0.55*_levDeflator);
...
(async function init(){
  ...
  //    that requires a server-side cron job, which is a separate, bigger change.
