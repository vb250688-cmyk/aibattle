...
const aiMemory = {
  ...
  learnFromHistory() {
    ...
    if      (consecLosses >= 5)                             this.mood = 'CAUTIOUS'; // ★ 5+ losses = CAUTIOUS not REVENGE
  ...
  sizePenalty(side) {
    ...
    if (this.mood === 'REVENGE')  mult *= 0.50; // ★ FIX: half size in losing streak
...
function checkAITrade(){
  ...
  const entryThreshBase=mkt.isChop?11:10;  // was 14 in CHOP — too strict, lowered to 11
  ...
  for(const coin of COINS){
    ...
    const MIN_SL=0.60;  // minimum SL = 0.60% (wider — avoid 1M noise stops)
    ...
    if(!hardBlockShort && aiMemory.shortsWorking()){
      ...
      if(canShort){
        ...
        // In trending market: thresh=13 | non-trending: thresh=14 (was 16/19 — too strict)
        const shortThresh = ind.trendingMarket
          ? Math.max(entryThresh+3+coinBoost, 10)
          : Math.max(entryThresh+4+coinBoost, 14);
        const shortAdxOk = ind.trendingMarket || shortScore >= 14; // ★ ADX filter for shorts
        const shortOk = shortScore>=shortThresh
          &&(ind.trend_signal==='STRONG_SELL'||ind.trend_signal==='SELL'||shortScore>=shortThresh+4)  // extreme score can bypass signal filter
          &&ind.htfTrend!==1
          &&ind.rsi>20  // only block if deeply oversold/capitulation (was 35 — too strict!)
          &&shortAdxOk;
        if(shortOk){
    ...
    // ══════════════════════════════════════════════════════════
    if(!hardBlockLong){
      const canLong=mkt.regime==='BULL'||mkt.regime==='STRONG_BULL'||mkt.isChop||mkt.regime==='BEAR'; // allow opportunistic longs in BEAR at extreme oversold (RSI<30)
      if(canLong){
        ...
        const bearLongAllowed = mkt.regime!=='BEAR'&&mkt.regime!=='STRONG_BEAR'
          || (ind.rsi<28 && ind.stochOversold && longScore>=entryThresh+3); // oversold bounce
        const longOk=canLong&&bearLongAllowed&&longScore>=entryThresh
          ...
          &&ind.rsi>18&&ind.rsi<86;  // allow entries up to RSI=85 (overbought but valid)
...
function executeAITrade(side, amount, coinId, meta={}) {
  ...
  if(side==='buy'){
    ...
    const grossPnl = actualQty * (price - h.avgCost); // leveraged gain/loss
    const pnl = grossPnl - exitFee;
    const margin = h.margin || h.invested || 0;
    // ✅ Return: margin + pnl (NOT full notional proceeds)
    state.aiCash += margin + pnl;
    delete state.aiHoldings[coinId];
    if(pnl>0){state.aiWins++;addAiPoints(calcPts(pnl, margin));}else{state.aiLosses++;}
    state.aiTradeLog.unshift({type:'sell',coin:coinId,qty:actualQty,price,pnl,fee:exitFee,time,
      leverage:AI_LEV, entryPrice:h.avgCost,
      entryRsi:meta.entryRsi||null, hadPattern:meta.hadPattern||false, regime:meta.regime||null});
...
function placeUserFuturesTrade(side,evt){
  ...
  if(side==='close_long'){
    ...
    // Futures exit: user gets back margin +/- pnl (not full notional)
    const exitFee   = pos.qty * price * 0.001;            // fee on notional at exit
    const grossPnl  = pos.qty * (price - pos.avgCost);    // leveraged gain/loss
    const pnl       = grossPnl - exitFee;
...
function updateUI(){
  ...
  logItems=logItems.concat(state.aiTradeLog.slice(0,5).map(t=>{
    ...
    const levStr=t.leverage?`<span style="font-size:9px;opacity:0.65;background:rgba(255,255,255,0.06);border-radius:3px;padding:0 3px;margin-right:2px">${t.leverage}x</span>`:'';
  ...
  let holdHtml=posEntries.map(([id,p])=>{
    ...
    // P&L based on qty movement (leveraged)
    const pnl    = isLong ? p.qty*(cur-p.avgCost) : p.qty*(p.entryPrice-cur);
    const pnlPct = (pnl / Math.max(margin,1)) * 100; // % of margin at risk
...
function showTierModal(){
  ...
  const tilesHtml = tiers.map(t => {
    ...
    return `<div style="background:${bgCol};border:1px solid ${borderCol};border-radius:8px;padding:10px 12px;margin-bottom:8px;cursor:${unlocked?'default':'pointer'}" onclick="${unlocked?'':'unlockTier('+t.id+')'}">
      ...
      ${!unlocked?'<div style="font-size:9px;color:var(--amber);margin-top:5px">⚡ Click to unlock via Sepolia gas tx (no ETH cost)</div>':''}
    </div>`;
  }).join('');
...
async function unlockTier(tierId){
  ...
  showToast(`Sending Sepolia tx to unlock ${tierName}...`, 'info');

  try {
    const provider = getMetaMaskProvider() || window.ethereum;
    // Send 0-value gas tx to our contract (Sepolia testnet)
    ...
    // Optimistic unlock — don't wait for confirmation
    if(!state.unlockedTiers) state.unlockedTiers = [];
    state.unlockedTiers.push(tierId);
    saveTiers();
    showToast(`🎉 Tier ${tierId} — ${tierName} UNLOCKED!`, 'success');
    // Refresh modal
    ...
    if(e.code===4001){
      ...
      // Even on error, unlock locally (gas may have been spent)
      if(!state.unlockedTiers) state.unlockedTiers = [];
      if(!state.unlockedTiers.includes(tierId)){
        state.unlockedTiers.push(tierId);
        saveTiers();
        showToast(`Tier ${tierId} unlocked (local)!`, 'success');
        setTimeout(showTierModal, 400);
      } else {
