...
function checkAITrade(){
  ...
  for(const coin of COINS){
    ...
    // Entries are gated off locally once a wallet is connected — the background
    // Supabase Edge Function (ai-trade-tick) becomes the sole entry-decision-maker
    // for that wallet so we never get duplicate/conflicting trades. Exits above
    // this line still run locally regardless (safe/idempotent).
