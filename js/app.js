      ...
      if(canShort){
        ...
        if(shortOk){
          ...
          if(isTierUnlocked(4)){
            ...
            riskAmt=state.aiCash*(0.03+confidence*0.05)*streakBoost*sizeMult;
          }
    ...
    if(!hardBlockLong){
      ...
      if(canLong){
        ...
        const bearLongAllowed = mkt.regime!=='BEAR'&&mkt.regime!=='STRONG_BEAR'
          || (ind.rsi<28 && ind.stochOversold && longScore>=entryThresh+3);
        ...
        const longOk=canLong&&bearLongAllowed&&longScore>=entryThresh
          ...
          &&ind.rsi>18&&ind.rsi<86&&t6LongOk;  // allow entries up to RSI=85 (overbought but valid)
        ...
        if(longOk){
          ...
          if(longScore < longThresh){ /* coin preference says skip */ }
            ...
            if(isTierUnlocked(4)){
              ...
              riskAmt=state.aiCash*(0.035+confidence*0.065)*streakBoost*sizeMult;
            }
...
async function unlockTier(tierId){
  ...
  try {
    ...
    // ✅ Tx sent — optimistic unlock (don't wait for block confirmation)
    if(!state.unlockedTiers) state.unlockedTiers = [];
    if(!state.unlockedTiers.includes(tierId)) state.unlockedTiers.push(tierId);
    saveTiers();
    showToast(`🎉 Tier ${tierId} — ${tierName} UNLOCKED on-chain!`, 'success');
    ...
    if(e.code===4001 || e.code===-32603){
      ...
      // Unknown error — unlock locally with warning
      showToast(`⚠️ TX error: ${e.message||'unknown'}. Unlocking locally.`,'warning');
      if(!state.unlockedTiers) state.unlockedTiers = [];
