  ...
  for(const coin of COINS){
    ...
    if(!hardBlockShort && aiMemory.shortsWorking()){
      ...
      if(canShort){
        ...
        // In trending market: thresh lower | STRONG_BEAR: even easier to short (right direction)
        const bearBonus = (mkt.regime==='STRONG_BEAR'||mkt.regime==='BEAR') ? -2 : 0;
        const shortThresh = ind.trendingMarket
          ? Math.max(entryThresh+3+coinBoost+bearBonus, 9)
          : Math.max(entryThresh+4+coinBoost+bearBonus, 13);
        ...
        if(shortOk){
          ...
          const margin=Math.max(150,Math.min(Math.floor(riskAmt*1.5),state.aiCash*0.58,340));
      ...
      if(canLong){
        ...
        if(longOk){
          ...
          if(longScore < longThresh){ /* coin preference says skip */ }
            ...
            const amount=Math.max(150,Math.min(Math.floor(riskAmt*1.5),state.aiCash*0.68,340));
