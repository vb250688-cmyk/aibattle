...
function checkAITrade(){
  ...
  for(const coin of COINS){
    ...
    // ★ FIX: entries used to be gated OFF entirely once a wallet connected,
    // on the assumption that the Supabase Edge Function (ai-trade-tick) would
    // take over as sole entry-decision-maker in the background. That edge
    // function is never actually invoked (no cron/scheduler calls it — see
    // "No recent invocations" in Supabase dashboard), so with the old check
    // the AI would just permanently stop opening new trades the moment a
    // wallet connected. Local entries now always run; if/when ai-trade-tick
    // is wired up to a real scheduler in the future, re-add a wallet check.
