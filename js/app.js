...
function checkAITrade(){
  ...
  if(mkt.regime==='STRONG_BULL'){
    for(const coinId in state.aiShorts){
      ...
      if(pct<=-0.4||holdSec>40){executeAITrade('close_short',null,coinId);delete aiTrailingStop[coinId];aiCooldown[coinId]=now;}
    }
  }
  if(mkt.regime==='STRONG_BEAR'){
    for(const coinId in state.aiHoldings){
      ...
      if(pct<=-0.4||holdSec>40){executeAITrade('sell',null,coinId);delete aiTrailingStop[coinId];aiCooldown[coinId]=now;}
    }
  }
  ...
  for(const coin of COINS){
    ...
    const cooldown=mkt.isChop?35000:20000; // 20-35s cooldown — no overtrading
    ...
    if(short){
      ...
      const exitShort=
        ...
        || (holdSec>180&&pct<FEE_RT);                 // ★ time-stop: 3min, only if not covering fees
    ...
    if(holding&&holding.qty>1e-6){
      ...
      const exitLong=
        ...
        || (holdSec>180&&pct<FEE_RT);                // ★ 3min time-stop — only if underwater
