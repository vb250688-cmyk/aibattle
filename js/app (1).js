...
// ---- INIT ----
(async function init(){
  console.log('[AIBattle] Init started — making UI responsive immediately');
  
  // Make UI responsive IMMEDIATELY (don't wait for data)
  updateUI();
  setInterval(updateUI,1000);
  
  // Load candles in background with timeout (max 8 seconds)
  const loadWithTimeout = Promise.race([
    loadAllCandles(),
    new Promise((_, reject) => setTimeout(() => reject(new Error('Candle load timeout')), 8000))
  ]);
  
  loadWithTimeout.catch(err => {
    console.warn('[AIBattle] Candle load failed/timeout:', err.message);
    // Still continue — app works with fallback prices
  });
  
  // Refresh prices in background (non-blocking)
  const refreshWithTimeout = Promise.race([
    refreshAll(),
    new Promise((_, reject) => setTimeout(() => reject(new Error('Refresh timeout')), 5000))
  ]);
  
  refreshWithTimeout.catch(err => {
    console.warn('[AIBattle] Refresh failed/timeout:', err.message);
  });
